...
fun GameScreen(
    ...
    when (configuration.orientation) {
        Configuration.ORIENTATION_PORTRAIT -> {
            MainActivityPortrait(modifier = modifier, viewModel = viewModel, onBack = onBack)
        }

        Configuration.ORIENTATION_LANDSCAPE -> {
            MainActivityLandscape(modifier = modifier, viewModel = viewModel, onBack = onBack)
        }
...
fun MainActivityLandscape(
    ...
    Row(modifier = modifier) {
        Keypad(modifier = Modifier.weight(1f), viewModel = viewModel)
        Column(
            modifier = Modifier
                .fillMaxHeight()
                .weight(1f)
            ...
            Submit(viewModel = viewModel, onBack = onBack)
        }
    }
}
